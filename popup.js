...
//js для попапа - тут логика с отправкой лида
$(function() {
    ...
    chrome.tabs.query({ active: true, windowId: chrome.windows.WINDOW_ID_CURRENT }, function(tabs)
    {
        ...
        $('.lead-address').html(url);
        $('.lead-name').html(title);

        let address = localStorage['address'];
        let webhook = localStorage['webhook'];

        if (!checkSite(url)) {
            $('.js-btn-send-lead').hide();
        }

        $('.js-btn-send-lead').on('click', function(){
            $.ajax({
                url: address + '/rest/1/' + webhook + '/crm.lead.add/?TITLE=1231',
                type: 'post',
                dataType: 'json',
                data: {'fields' : {'TITLE' : title, 'WEB' : [{'VALUE': url, "VALUE_TYPE": "HOME"}]}},
                success: function (data) {
                    $('.js-btn-send-lead').hide();
                    $('body').append('<span style="color: green">Отправлено</span>');
                    //console.log(data);
                }
        ...
        {
            let regex = /facebook\.com\//gu;
            let m;
            return (m = regex.exec(site)) !== null;
        }
